...
public class DayInMonth {
    public static void main(String[] args) {
        ...
        while (month == -1) {
            System.out.print("Enter a month (Full name, Abbr., 3 letters, Number): ");
            String input = sc.nextLine().trim();
            month = getMonthFromInput(input);
            if (month == -1) {
        ...
        while (year < 0) {
            System.out.print("Enter a non-negative year (e.g., 1999): ");
            String input = sc.nextLine().trim();
            try {
                if (input.length() >= 4 && (year = Integer.parseInt(input)) >= 0) {
                } else {
                    System.out.println("Invalid year format. Must be a non-negative number with all digits.");
                    year = -1;
                }
    ...
    private static int getMonthFromInput(String input) {
